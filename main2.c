...
int main() {
  ...
  /*Robot runs program until the specified amount of bunkers are claimed. Below is the strategy:
    1. Move forward until a sensor reads black underneath it (a value of 1)
    2. Once a sensor reads 1, have the robot sweep over the source of the black tape
      a. if when sweeping, 4 or more sensors read 1 at the same time, the source must be a wall
        Then it will turn around at a random angle and continue moving forward again
      b. if when sweeping, less than 4 sensors read 1 at the same time, the source must be a bunker.
        Then the buzzer will go off and it will back up from the bunker, turn around, and wait for a moment. It will then continue moving forward
        The bunker count will go up by one after finding a bunker. If it was the last bunker, then it will back up from the bunker, spin twice and beep
    While the robot is going, the sensor values will be displayed along with how many bunkers have already been found
  */
